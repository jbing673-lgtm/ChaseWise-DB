...
function RoundItem({ round, isLast }: { round: Round; isLast: boolean }) {
  ...
  const responseLabel = OPPONENT_RESPONSE_LABELS[round.opponent_response as keyof typeof OPPONENT_RESPONSE_LABELS] || round.opponent_response_custom || round.opponent_response;

  return (
    <div className="card">
      <div className="flex items-center justify-between cursor-pointer" onClick={() => setExpanded(!expanded)}>
        <div className="flex items-center gap-3"><span className="text-lg font-bold text-gray-900">Round {round.round_number}</span><TierBadge tier={round.tier} /></div>
      ...
      {round.opponent_response && (<div className="mt-3 pt-3 border-t border-gray-100"><p className="text-sm text-gray-500">Their last response:</p><p className="mt-1 text-sm text-gray-700">{responseLabel}</p></div>)}
      {expanded && (
        <div className="mt-4 pt-4 border-t border-gray-100">
          <p className="text-sm font-medium text-gray-700 mb-2">Generated Email:</p>
          <div className="bg-gray-50 rounded-lg p-4 text-sm text-gray-700 whitespace-pre-wrap leading-relaxed">{round.generated_email_en}</div>
          {round.rebuttals && round.rebuttals.length > 0 && (
            <div className="mt-4 pt-4 border-t border-gray-100">
              <p className="text-sm font-medium text-gray-700 mb-2">Rebuttal Suggestions:</p>
              <ul className="space-y-2">{round.rebuttals.map((reb, i) => (<li key={i} className="text-sm text-gray-600 bg-gray-50 p-3 rounded-md">{i + 1}. {reb}</li>))}</ul>
            </div>
          )}
        </div>
      )}
